...
# In owr case, we will use just the local docker enviornment to avoid virtualization
ADDRESS="$( docker container port xmart-notebook | cut -d ' ' -f 3 )"
echo "The Jupyter Notebook is available on: '$ADDRESS'"

# This step is to wait until the jupyter starts 
while [[ ${TOKEN#0} == "" ]]; do
TOKEN="$( docker exec -it $NAME jupyter notebook list | grep http | cut -d ' ' -f 1 | cut -d '=' -f 2 )"
done
echo "And the token to access is: '$TOKEN'"
